...
def zonas_wifi():
    ...
    informacion['actual'] = ubicacion_actual
    ...
    if(indi == w1 or indi == w2):
        ...
        informacion['zonawifi1'] = zonas[indi-1]
        tiempo_pro_auto = tiempo_promedio(distancias_orden[indi][1], "auto")
        tiempo_pro_bici = tiempo_promedio(distancias_orden[indi][1], "bici")
        informacion['recorrido'] = [
            distancias_orden[indi][1], "auto", tiempo_pro_auto
        ]
        print(
        ...
        print(
            f'Tiempo en auto {tiempo_pro_auto}s')
        print(
            f'Tiempo en bicicleta {tiempo_pro_bici}s')
...
def guardar_informacion():
    ...
    if((coordenadas[0][0] == 0) or informacion['actual'] == None):
        print('Error de alistamiento')
        return False

    print(f'Ubicación actual: {informacion["actual"]}')
    print(f'Zona Wifi: {informacion["zonawifi1"]}')
    distancia, mediotransporte, tiempopromedio = informacion["recorrido"]
    print(
        f'Recorrido: \n=> Distancia: {distancia}\n=> Medio de trasnporte: {mediotransporte}\n=> Tiempo promedio: {tiempopromedio}'
    )
    save = int(
        input('¿Está de acuerdo con la información a exportar?\nPresione 1 para confirmar, 0 para regresar al menú principal\n')
    )
    if(save == 1):
...
informacion = {
    'actual': None,  # [latitud, longitud]
    'zonawifi1': None,  # [latitud, longitud, usuarios]
    'recorrido': None,  # [distancia, mediotransporte, tiempopromedio]
}
rootOpciones = [
    ['Cambiar contraseña', change_password],
    ['Ingresar coordenadas actuales', input_coordenadas],
    ['Ubicar zona wifi más cercana', zonas_wifi],
    ['Guardar archivo con ubicacción cercana', guardar_informacion],
    ['Actualizar registros de zonas wifi desde archivo', actualizar_zonas],
